...
var fileIndex = 1;
// both version and manifest keep track of how many video versions there are;
...
// generates correct file path name for .m3u8 files
function getName(videoName){
  let name = (videoName+'_'+manifestCounter)
  manifestCounter++;
  return (name)
}

function createManifestText (manifest, rootUri,videoName) {
  manifestCounter=0;
  return manifest.map(function (line) {
    if (line.type === 'playlist') {
      var subCWD = getName(videoName);
...
function getIt (options, done) {
  var uri = options.uri;
  var cwd = options.cwd;
  // added avility to choose name
  var videoName=(options.videoName)
  ...
  fetch.fetchUrl(uri, {timeout: DEFAULT_TIMEOUT}, function getPlaylist (err, meta, body) {
    ...
    if (playlistFilename.match(/\?/)) {
      ...
      // Named .m3u8 to correct Name
      playlistFilename = (videoName+'.m3u8');
    }
    if (!playlistFilename.match(/.+m3u8$/i)) {

      playlistFilename = "playlist" +  ".m3u8";
    }
    fs.writeFileSync(path.resolve(cwd, playlistFilename), createManifestText(manifest, uri,videoName));
    ...
    async.series([
      ...
      function fetchPlaylists (next) {
        async.eachSeries(playlists, function (resource, done) {
          ...
          var subCWD =( videoName+'_'+versionCounter);
          var subDir = path.resolve(cwd, subCWD);
          // adds 1 to version counter to correctly name file
          versionCounter++;
